...
def iterate_minibatches(*tensors, batch_size, shuffle=True, epochs=1,
    ...
    """
    Minibatch iterator that's faster than torch dataloader when dealing with
    very simple objects and hundreds of batches per second.
    :param tensors: tensors to iterate over, all tensors must have shape [dataset_size, ...]
        compatible with np arrays, torch tensors and anything that can be indexed via tensor[[i0, i1, i2]]
    :param batch_size: maximum number of objects in one yield
    :param shuffle: if True, each epoch runs over data tensors in shuffled order
    :param epochs: number of full passes over train data
    :param allow_incomplete: if dataset_size is not divisible by batch_size,
        if True, the last batch in epoch will have less than batch_size objects
        if False, the last batch in epoch will be omitted
    :param callback: a wrapper for batch iterator, insert your tqdm/progressbar here :)
    """
    ...
    while True:
        ...
        for batch_start in callback(range(0, upper_bound, batch_size)):
            if batch_start == len(indices): break
...
def process_in_chunks(function, *args, batch_size, out=None, **kwargs):
    ...
    if out is None:
        out = torch.empty(*output_shape, dtype=first_output.dtype, device=first_output.device,
